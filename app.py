    ...
    def calculate(self):
        ...
        while calculated_anything:
            ...
            calculations = [
                ...
                {
                    'param': 'mr',
                    'func': lambda mass, moles: mass / moles,
            ...
            for calculation in calculations:
                calculated_anything |= self.try_calculate_value(inputs, calculation['param'], calculation['func'], calculation['equation'], calculation['params'])

    def try_calculate_value(self, inputs, param, func, equation, params):
        if param in inputs and inputs[param] is not None:
            return False  # Skip if this parameter is already known
        ...
        if all(p in inputs and inputs[p] is not None for p in params):
            ...
            return True  # Indicate that a calculation was made
        
        return False  # No calculation made


class BoilingPointCalculator:
    def __init__(self):
        st.title("زۆرترین نیشانی ژمێرکاری: ژمێرکاری بۆ تواوەی نا ئەلیکترۆلیتی")
        self.create_layout()
...
def main():
    st.sidebar.title("کاتی ژمێرکاری")
    page = st.sidebar.selectbox("بەڵگە:", ("ژمێرکاری بۆ توێنەر", "ژمێرکاری بۆ تواوەی نا ئەلیکترۆلیتی"))

    if page == "ژمێرکاری بۆ توێنەر":
